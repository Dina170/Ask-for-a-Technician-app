...
// يولّد public/sitemap.xml عبر زحف خفيف لبنفس الدومين.
...
// ========= إعدادات بسيطة =========
const BASE_URL = 'https://www.imadaldin.com';   // عدّل الدومين لو لزم
...
async function crawl(startUrl) {
  ...
  const urls = new Map(); // loc -> Date lastmod
  ...
  while (queue.length && visited.size < MAX_PAGES) {
    ...
    try {
      ...
      // استخراج روابط داخلية سريعة (نمط href="")
      const hrefs = Array.from(html.matchAll(/href\s*=\s*"(.*?)"/gi))
        .map(m => m[1])
        .filter(Boolean);

      for (const h of hrefs) {
        // تجاهل mailto: وtel: والأنكور الداخلي
